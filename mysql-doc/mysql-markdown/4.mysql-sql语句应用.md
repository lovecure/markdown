...
> 索引大概有三种: 
    ...
    (1) 主键索引 PRI:主键列要求所有内容必须 唯一 每个表只能有一个主键索引容
        ;如同学生在学校中的学号

>> <pre>不论是主键索引还是普通索引,都要在
>> <mark>表的对应列上创建,可以对单列创建索引,也可以对多列创建索引 </mark></pre> 
...
<br>
    ...
    (2) 普通索引 MUL:
        2.1 唯一普通索引(不常用)
>> 希望表中某个列内容是唯一的,但是这个表中已经存在主键,表中又只能存在一个主键,这时可以创建 唯一普通索引
...
<mark>注意:</mark>同样,普通索引在建表时如果没有建立可以使用alter来创建

语法:
1.删除之前创建的普通索引:
  
  alter table student drop index <makr>index_name 索引名称</mark>;
  
2.添加普通索引:
  
  alter table student add <mark>index 固定语法</mark> index_name(列名称);
