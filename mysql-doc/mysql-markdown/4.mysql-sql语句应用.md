...
命令语法:
创建用户并授权
 GRANT ALL PRIVILEGES  ON db_name.tables TO 'user'@'host' IDENTIFIED BY 'password'; 
...
| grant | all privileges | on dbname.tables| to  ‘username’@‘host’  | identified by ‘password' |
| :-- | :-- | :-- | :-- | :-- |      
...
> 索引大概有三种: 
    ...
    (1) 主键索引:主键列要求所有内容必须唯一, 每个表只能有一个主键索引容 ;如同学生在学校中的学号

>> <pre>不论是主键索引还是普通索引,都要在<mark>表的对应列上创建,可以对单列创建索引,也可以对多列创建索引 </mark></pre> 
...
(id int(4) not null <mark>AUTO INCREMENT</mark>自增
,name char(20) not null,age tinyint(2) not null default '0',dept varchar(16) 
default null,<mark>primary key(id)</mark>, <mark>KEY index_name (name)</mark>);


<mark>primary key</mark> <--- 主键
<mark>KEY index_name (name)</mark> <---  字段普通索引
